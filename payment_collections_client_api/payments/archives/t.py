...
def process_payment_request(provider, account, amount,  payer_ref,
    ...
    try:
        ...
        if valid_provider and provider == "VODAFONE":
        ...
        elif valid_provider and provider == "MTN" or valid_provider and provider == "AIRTELTIGO":
            sender_id = settings.SMS_SOURCE_ADDRESS
            pload = {
            "account_number": account,
            "amount": amount,
            ...
            text = f"""Dear customer,we are processing your payment of GHS.{amount} via {provider} to {sender_id}-Ref:{payer_ref}."""
