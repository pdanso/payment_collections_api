...
def loans(request, msg, bank_code, last_position, pos, goback_message):
    ...
    if last_position == "LONSTA":
        ...
        if userdata == "1":  # Request for Loan
            message = "Terms and Conditions:^^Monthly Interest rate of 3.5%.^" \
            ...
            message = "Interest rate of 3.5%/month.^" \
                      "4.5% of loan amount charged as processing fee.^Visit loans.mesika.org/ts to review " \
                      "this in full."
    ...
    # ================ Session begins ========================================
        ...
        message = "4.5% of loan amount as processing fee.^Visit loans.mesika.org/ts to review this in " \
                  "full.^Enter any digit to continue."
        menu_response = core_processor.make_response.make_response(request, "more", message)
        ...
        if cust_status == "INTERESTED" or cust_status == "INTEREST":
            ...
            res = {"status": 200, "id_type_list": [{"id_uuid": 12332, "id_name": "Voters ID"},
                                                   {"id_uuid": 12232, "id_name": "Passport"},
                                                   {"id_uuid": 10332, "id_name": "Drivers License"},
                                                   ]}
            ...
            if status == 200:
                ...
                menu_response = core_processor.goto_start(request, message, stored_data, goback_message)
                loans_processor.libhandler.writelog(logfile, f"Message = {message}")
    ...
    # ================= Completing loan request ============================== """
        ...
        if tenure <= 0 or tenure > 36:
            ...
            if status == 200:
                ...
                menu_response = core_processor.goto_start.goto_start(request, message, status, goback_message)
                loans_processor.libhandler.writelog(logfile, f"Message = {message}")

    elif last_position == "LONCNF":
        loans_processor.libhandler.writelog(logfile, f"Extract: welcome")
        extract = session_processor.get_ussd_extra.get_ussd_extra(pos)
        extract_reply = extract.split('|')
        ...
        if userdata == "1":
            ...
            menu_response = core_processor.goto_start.goto_start(request, message, cust_status, goback_message)
            loans_processor.libhandler.writelog(logfile, f"Message = {message}")
