...
def momo_debit(request, url, bank_code, data, last_position, goback_message, pos):
    ...
    if last_position == "MDBCHG":  # list all the momo channels available
        ...
        charge = float(amount) * float(mtndebit)
        totalcharge = charge + float(amount)
        message = f"Charge is {charge}. Your {momo_name} account {momo_num} will be debited with Ghc {totalcharge} to send to account number" \
                  f" {acct_num}.^" \
