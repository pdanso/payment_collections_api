...
def momo(request, url, bank_code, data, last_position, goback_message, pos):
    ...
    if last_position == "MOMSUB":
        ...
        if subservice_id == 1:  # bank to wallet
            menu_response = momo_processor.bank_wallet.momo_credit(request, "", bank_code, 3,
                                                                   "MCRCHG", goback_message, pos)

        elif subservice_id == 2: # momo to wallet
            menu_response = momo_processor.wallet_bank.momo_debit(request, "", bank_code, 3, "MDBCHG",
                                                                  goback_message, pos)
        ...
        menu_response = momo_processor.wallet_bank.momo_debit(request, "", bank_code, 3,
                                                              last_position, goback_message, pos)
