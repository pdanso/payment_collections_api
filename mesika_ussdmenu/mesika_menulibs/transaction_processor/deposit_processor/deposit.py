...
#        service_id = ""
        ...
        if userdata == "1":
            ...
            message = f"Enter Receipt Number" 
            menu_response = core_processor.make_response.make_response(request, "more", message)
            session_processor.store_menupoint.store_menupoint(request, "DEPRCT", stored_data)
            deposit_processor.libhandler.writelog(logfile, f"Message {message}")
    ...
    elif last_position == "DEPRCT":
         message = f"You will receive a response shortly. Your Ref ID is: 93849302930" 
         menu_response = core_processor.goto_start.goto_start(request, message, "", goback_message)
         deposit_processor.libhandler.writelog(logfile, f"Message {message}")
