...
def internal(request, endpoint, bank_code, data, last_position, goback_message, pos):
    ...
    if last_position == "FTRACL":
        ...
        if not credit_acct_num.isdigit():
            ...
            url = f"{endpoint}/customers/search/account_number/{credit_acct_num}/"
            headers = {"X-CUSTOMER-AUTH-TOKEN": token}
        ...
        if not amount.isdigit():
            ...
            if len(userdata) < 1 or userdata == "0":
                ...
                status = 200
                response['has_funds'] = 1
                if status == 200 and response['has_funds'] == 1:
                    #balance = response['balance']
                    balance = 0
                    message = f"Your current balance is GHC {balance}^Your account {account_number} will be debited with Ghc{amount} " \
