    ...
    if last_position == "FNDACL":
        ...
        if not amount.isdigit():
            ...
            if len(userdata) < 1 or len(userdata) > 4 or userdata == "0":
                ...
                message = f"Your account will be debited with Ghc{amount} to credit account number {credit_acct_num}." \
        ...
        if userdata == "1":
            ...
            status = 200
            if status == 200:
                message = f"Funds transfer of Ghc {amount} to account number {credit_acct_num} is successful."
                menu_response = core_processor.goto_start.goto_start(request, message, data, goback_message)
                ft_processor.libhandler.writelog(logfile, f"Message: {message}")
