    ...
    if last_position == "WTDBNK":
        ...
        if status == 200:
            ...
            if account_results['status'] == 200:
                ...
                if acc_count >= 1:
                    ...
                    menu_response = core_processor.make_response.make_response(request, "more", message)
                    session_processor.store_menupoint.store_menupoint(request, "WTDACC", stored_data)
                    withdrawal_processor.libhandler.writelog(logfile, f"Message: {message}")
        ...
        if not amount.isdigit():
            ...
            if len(amount) < 1 or len(amount) > 4 or amount == "0":
                ...
                message = f"Transaction ID and Paycode will be generated and sent to {mbank_num} to withdraw Ghc" \
        ...
        if userdata == "1":
            ...
            message = "Transaction ID and pay code will be " \
                      "sent to your phone shortly"
            menu_response = core_processor.goto_start.goto_start(request, message, "", goback_message)
            withdrawal_processor.libhandler.writelog(logfile, f"Message {message}")

        elif userdata == "2":
            message = "Pay code cancelled. Transaction cannot be completed"
            menu_response = core_processor.goto_start.goto_start(request, message, "", goback_message)
            withdrawal_processor.libhandler.writelog(logfile, f"Message {message}")
