...
def view(request):
    ...
    try:
        if mode == "start":
            ...
            message = f"Welcome to CHARISMATIC EVANGELISTIC MINISTRY Payment Service.^Select:^{message}"
                ...
                if userdata == "1":
                    if pay_id == "1" or pay_id == "3":
                        ...
                        payload = {"reference_number": trxid, "account_number": momonum, "payer_msisdn": msisdn,
                                   "billed_amount": amount, "processing_fees": charge, "provider": pay_name,
